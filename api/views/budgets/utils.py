...
from api.services.budgets.budget_service import BudgetService, BudgetListResult, BudgetMonthSummary, MonthlyBudgetsResult
...
def render_budget_htmx_response(request: HttpRequest, year: int, month: int, include_messages: bool = False) -> HttpResponse:
    ...
    summary_html = render_to_string('budget/components/budget_summary.html',
                                    {**summary_context.to_context(), 'hx_oob': True},
                                    request=request)

    # Render the main card
    main_card_html = render_to_string('budget/components/budget_main_card.html',
                                      {**summary_context.to_context(), 'hx_oob': True},
                                      request=request)
